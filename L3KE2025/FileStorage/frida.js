...
Java.perform(() => {
  ...
  RootDetection.isDeviceRooted.implementation = function () {
    send('isDeviceRooted() got called! Let\'s call the original implementation');
    return false;
  };
});
...
const fwrite = Module.findGlobalExportByName("fwrite");
...
Interceptor.attach(fwrite, {
    onEnter(args) {
        ...
        if (args[1] == 1){
            console.log(buf.readUtf8String(args[2]))
        }
    }
});
